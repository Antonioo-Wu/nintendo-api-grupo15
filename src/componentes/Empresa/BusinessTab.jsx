...
function BusinessTab() {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/business/games/analytics`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
          }
  ...
  const confirmActionHandler = async () => {
    if (confirmAction.type === "eliminar") {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/business/games/${confirmAction.gameId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
  ...
  const handlePublishUnpublish = async (gameId, isPublished) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/business/games/${gameId}/${action}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
